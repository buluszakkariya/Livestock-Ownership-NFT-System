...
(define-map vaccination-records
    { token-id: uint, vaccination-id: uint }
    {
...
(define-map vaccination-schedule
    (string-ascii 30)
    {
...
(define-data-var base-health-score uint u100)

(define-constant err-unauthorized-vet (err u116))
(define-constant err-vaccination-exists (err u117))
(define-constant err-invalid-schedule (err u118))
(define-constant err-insufficient-rewards (err u119))
(define-constant err-early-vaccination (err u120))
...
(define-public (register-vaccination
    ...
    (let
        ((vaccination-id (var-get next-vaccination-id))
         (owner (unwrap! (nft-get-owner? livestock-token token-id) err-invalid-token))
         (schedule (map-get? vaccination-schedule vaccine-type))
         (current-health (default-to 
             {current-score: (var-get base-health-score), last-updated: u0, vaccination-count: u0, compliance-streak: u0}
             (map-get? health-scores token-id)))
         (new-score (+ (get current-score current-health) u10))
         (new-count (+ (get vaccination-count current-health) u1))
         (new-streak (+ (get compliance-streak current-health) u1)))
        (asserts! (or (is-eq tx-sender contract-owner) (is-eq tx-sender owner)) err-unauthorized-vet)
        (begin
            (var-set next-vaccination-id (+ vaccination-id u1))
            (map-set vaccination-records { token-id: token-id, vaccination-id: vaccination-id } {
                vaccine-type: vaccine-type,
                administered-by: tx-sender,
                vaccination-date: stacks-block-height,
                next-due-date: (+ stacks-block-height 
                    (match schedule
                        schedule-data (get interval-blocks schedule-data)
                        u1000)),
                batch-number: batch-number
            })
            (map-set health-scores token-id {
                current-score: (if (> new-score u200) u200 new-score),
                last-updated: stacks-block-height,
                vaccination-count: new-count,
                compliance-streak: new-streak
            })
            (unwrap-panic (distribute-health-reward owner vaccine-type))
            (ok vaccination-id))))
...
(define-public (set-vaccination-schedule
    ...
    (begin
        (asserts! (is-eq tx-sender contract-owner) err-owner-only)
        (map-set vaccination-schedule vaccine-type {
...
(define-private (distribute-health-reward (recipient principal) (vaccine-type (string-ascii 30)))
    (let ((schedule (map-get? vaccination-schedule vaccine-type)))
        (match schedule
            schedule-data 
                (let ((reward-amount (get reward-amount schedule-data)))
                    (if (and (> reward-amount u0) (>= (var-get reward-pool) reward-amount))
                        (begin
                            (var-set reward-pool (- (var-get reward-pool) reward-amount))
                            (map-set health-rewards recipient 
                                (+ (default-to u0 (map-get? health-rewards recipient)) reward-amount))
                            (ok reward-amount))
                        (ok u0)))
            (ok u0))))

(define-public (claim-health-rewards)
    (let ((reward-amount (default-to u0 (map-get? health-rewards tx-sender))))
        (asserts! (> reward-amount u0) err-insufficient-rewards)
        (try! (as-contract (stx-transfer? reward-amount tx-sender tx-sender)))
        (map-delete health-rewards tx-sender)
        (ok reward-amount)))
...
(define-public (update-health-score-decay)
    (let ((token-id u1))
        (match (map-get? health-scores token-id)
            health-data
                (let ((blocks-since-update (- stacks-block-height (get last-updated health-data)))
                      (decay-amount (/ blocks-since-update u100)))
                    (if (> blocks-since-update u1000)
                        (map-set health-scores token-id
                            (merge health-data {
                                current-score: (if (> decay-amount (get current-score health-data)) 
                                                   u50 
                                                   (- (get current-score health-data) decay-amount)),
...
(define-read-only (get-vaccination-record (token-id uint) (vaccination-id uint))
    (map-get? vaccination-records { token-id: token-id, vaccination-id: vaccination-id }))

(define-read-only (get-health-score (token-id uint))
    (map-get? health-scores token-id))

(define-read-only (get-vaccination-schedule-info (vaccine-type (string-ascii 30)))
    (map-get? vaccination-schedule vaccine-type))

(define-read-only (get-pending-rewards (user principal))
    (default-to u0 (map-get? health-rewards user)))

(define-read-only (get-reward-pool-balance)
    (var-get reward-pool))

(define-read-only (calculate-insurance-discount (token-id uint))
    (let ((health (map-get? health-scores token-id)))
        (match health
            health-data
                (let ((score (get current-score health-data)))
                    (if (>= score u150) u20
                        (if (>= score u120) u10
                            (if (>= score u100) u5 u0))))
            u0)))
